...
from app.services.memory import get_messages, save_message, update_conversation_title
...
async def stream_chat(data: ChatRequest, db: AsyncSession) -> AsyncGenerator[str, None]:
    ...
    Checked conversation existence at router.
    """
    try:
        ...
        await save_message(
            db, data.conversation_id, "assistant", "".join(full_response)
        )
    except APIError as e:
        yield _sse({"error": f"AI provider error: {e.message}"})
        return
